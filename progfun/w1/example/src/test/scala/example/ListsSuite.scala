...
class ListsSuite {
  ...
  @Test def `sum of zeros`: Unit = {
    assert(sum(List(0, 0, 0)) == 0)
  }

  @Test def `max of zeros`: Unit = {
  ...
  @Test def `sum of repeated elements`: Unit = {
    assert(sum(List(10, 10, 10)) == 30)
  }

  @Test def `max of repeated elements`: Unit = {
    assert(max(List(10, 10, 10)) == 10)
  }
