...
trait Solver extends GameDef {
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): LazyList[(Block, List[Move])] = {
    for (
      (block, move) <- b.legalNeighbors.to(LazyList) // no point to do List -> LazyList, but OK
  ...
  def newNeighborsOnly(neighbors: LazyList[(Block, List[Move])],
    ...
    case (block: Block, _) => !explored.contains(block)
  })
  ...
  def from(initial: LazyList[(Block, List[Move])],
    ...
    if (initial.isEmpty) LazyList.empty
      ...
      val more = for {
        (prev_block, prev_history) <- initial
        (block, history) <- newNeighborsOnly(neighborsWithHistory(prev_block, prev_history), explored)
      } yield (block, history)
